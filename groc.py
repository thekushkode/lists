...
1. Print Grocery & Status List
2. Print One Index
3. Exit

'''

#THIS CODE WORKS THE LARGE EXERCISES
while True:
    ...
    if menu_choice == 4:
        ...
        if delete_choices == 3:
            ...
            delete_choices = int(input(delete_menu))
            del boo[start_index : end_index]
        if delete_choices == 4:
            menu_choice = int(input(main_menu))
    boo_val_list = []
    if menu_choice == 5:
        boo_change = int(input('Enter the index number to update: '))
        boo_val = input('You obtained this item, True or False: ')
        boo[boo_change] = boo_val
        print(boo)
    if menu_choice == 6:
        break
